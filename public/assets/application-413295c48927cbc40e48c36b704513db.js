...
(function() {
  $(document).ready(function() {
    return $("#ingredientsinput").keyup(function() {
      var tempValue;
      tempValue = $(this).val();
      $.ajax({
        url: '/ingredient',
...
(function() {
  var IngredientN, categoryN, stepN;
  ...
  $(document).ready(function() {
    ...
    $("body").on("click", ".modal-image", function(event) {
      ...
      return $('#Modal' + event.target.id).modal('hide');
    });
    ...
    $("#add_step").on("click", function() {
      $('#add_step').before('<div  name=\'S[]\'><legend>Step: <span class="number"> ' + stepN + '\ </span> <div class="btn btn-default open_modal" data-toggle="modal" data-target="#Modal' + stepN + '" id ="' + stepN + '" >Add Instagram content</div> <div class="modal fade" id="Modal' + stepN + '" role="dialog"> <div class="modal-dialog"> <div class="modal-content"> <div class="modal-header"> <h4 class="modal-title">Select a photo</h4> </div> <div class="modal-body"> <div class="row" id="modal-body_' + stepN + '"> <div id="modal_' + stepN + '"> </div> </div> </div> </div> </div> </div></legend> <input type="text" class="step" placeholder="Image URL" name="SM[]"" id= "SM_' + stepN + '"/>' + '<input required type=\'text\' class=\'step\' placeholder=\'Title for step ' + stepN + '\' name= \'SN[]\' id= \'SN' + stepN + '\'/>' + '<textarea required type=\'text\' class=\'step\' placeholder=\'Describe step ' + stepN + '\' name= \'S[]\' id= \'S' + stepN + '\'/></div>' + '<label for="inputsm">Time (minutes):</label>' + '<input type="range" value="30" name="points" min="0" max="180" class="change-duration"><br>' + '<input class="recipe-form recipe_duration" value="30" min="0" max="180" id="inputsm" name="T[]" type="number">');
      return stepN++;
    });
    ...
    $("body").on("click", ".cat", function(event) {
      var catValue, realID;
    ...
    $("#book-btn").on("click", function() {
      return $("#bookmark_ico").first().addClass('fa-spinner fa-pulse').removeClass('fa-bookmark');
    });
    $("body").on("click", ".SIResult", function(event) {
      var IngID, IngQuant;
    ...
    $("#ingredients-input").keyup(function() {
      ...
      if (tempValue !== "") {
        return $.ajax({
          ...
          type: 'post',
          data: {
            ingredients: tempValue
          },
          success: function(response) {
            var ing;
            ing = $(response).find('#div-ing').html();
            return $('#search-result').html(ing);
          }
        });
      }
    });
    $(".remove_ingredient").on("click", function(e) {
      ...
      return $("#ingredient" + realID).remove();
    });
    $('body').on('change', '.change-duration', function(event) {
      return $('.recipe_duration').val($(this).val());
    });
    $('body').on('change', '.recipe_duration', function(event) {
      return $('.change-duration').val($(this).val());
    });
    $("#edit_recipe_name").on("click", function() {
      $("#current_recipe_name").before("<legend id='recipe_name_label'> Recipe name:</legend>" + "<input required type=\'text\' name= \'new_name\' id= \'input_new_recipe_name\' value=\'" + $("#name").text() + "\' />" + "<button type='button' id='button_change_name_done' class='btn btn-default'><i class='fa fa-check-square'></i></button>");
      $("#current_recipe_name").remove();
      $("#edit_recipe_name").remove();
      return $("#button_change_name_done").on("click", function() {
        var new_name;
        new_name = $("#input_new_recipe_name").val();
        $("#input_new_recipe_name").before('<input type="hidden" name="new_name" value="' + new_name + '">' + '<legend> <p> <div id="current_recipe_name"> ' + new_name + ' </div> <button type="button" id="edit_recipe_name" class="btn btn-default"><i class="fa fa-pencil-square-o"></i></button> </p> </legend>');
        $("#input_new_recipe_name").remove();
        $("#recipe_name_label").remove();
        return $("#button_change_name_done").remove();
      });
    });
    return $("#edit_recipe_description").on("click", function() {
      $("#current_recipe_description").before("<legend id='recipe_name_description'> Recipe description:</legend>" + "<textarea rows='12' cols='60' name= \'new_description\' id= \'input_new_recipe_description\'>" + $("#description").text() + "</textarea>" + "<button type='button' id='button_change_description_done' class='btn btn-default'><i class='fa fa-check-square'></i></button>");
      $("#current_recipe_description").remove();
      $("#edit_recipe_description").remove();
      return $("#button_change_description_done").on("click", function() {
        var new_description;
        new_description = $("#input_new_recipe_description").val();
        $("#input_new_recipe_description").before('<input type="hidden" name="new_description" value="' + new_description + '">' + '<legend> <p> <div id="current_recipe_description"> ' + new_description + ' </div> <button type="button" id="edit_recipe_description" class="btn btn-default"><i class="fa fa-pencil-square-o"></i></button> </p> </legend>');
        $("#input_new_recipe_description").remove();
        $("#recipe_description_label").remove();
        return $("#button_change_description_done").remove();
      });
    });
  });
...
(function() {
  ...
  $(document).ready(function() {
    ...
    $('[id^=chrBtn]').on('click', function() {
      var liftOff, liftoffTime, stepId;
      if ($('#countdown-' + this.id.split('-')[1]).length) {
        stepId = this.id.split('-')[1];
        ...
        $('#startButton').click(function() {
          ...
          $(this).text('Restart');
          $('#countdown-' + stepId).countdown('option', {
            until: liftoffTime
          });
        });
        $('#pauseButton').click(function() {
          ...
          $('#countdown-' + stepId).countdown(pause ? 'pause' : 'resume');
        });
      }
    });
  });
